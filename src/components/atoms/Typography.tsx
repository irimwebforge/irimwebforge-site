...
const accentWords = [
  'innovation', 'excellence', 'qualité', 'créativité', 'expertise',
  'efficacité', 'personnalisé', 'sur-mesure', 'unique', 'exclusif',
  'authenticité', 'digitale', 'artisanal', 'immersion'
];
  ...
  ({ className, variant, as, withAccentedWords, children, ...props }, ref) => {
    const Component = as ||
      (variant === 'h1' ? 'h1' :
       ...
       variant === 'h3' ? 'h3' :
       variant === 'h4' ? 'h4' :
       variant === 'small' || variant === 'subtle' || variant === 'accent' || variant === 'highlight' ? 'span' : 'p');
    ...
    if (withAccentedWords && typeof children === 'string') {
      ...
      accentWords.forEach(word => {
        ...
        while (startIndex !== -1) {
          ...
          parts.push(<span key={startIndex} className="text-[var(--color-tertiary)] font-medium">{originalWord}</span>);
