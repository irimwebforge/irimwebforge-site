...
export const ServiceHighlight: React.FC<ServiceHighlightProps> = ({
  ...
  return (
    <div className={`service-highlight surface-primary ${variantClasses.container} ${className}`}>
      {/* Icône */}
      {icon && (
        <div 
          className={`${variantClasses.iconContainer} ${colorClasses.bg} rounded-full flex items-center justify-center ${colorClasses.text}`}
        >
          {icon}
        </div>
      )}
      ...
      <Typography
        ...
        className={`${variantClasses.titleSize} font-bold ${variantClasses.spacing} ${colorClasses.text}`}
      >
        {title}
      </Typography>
      ...
      <Typography
        variant="p"
        className={`text-secondary ${bulletPoints.length > 0 ? 'mb-4' : ''}`}
      >
        {description}
      </Typography>
        ...
        <ul className="mt-3 mb-4 space-y-2">
          {bulletPoints.map((point, index) => (
            <li key={index} className="flex items-start">
              <span className={`mr-2 mt-1 inline-block w-1.5 h-1.5 rounded-full ${colorClasses.bg} ${colorClasses.border} border`}></span>
