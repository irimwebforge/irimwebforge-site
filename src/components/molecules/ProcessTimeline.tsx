        ...
        >
          <div className="flex flex-col items-center">
            ...
            <div className="relative z-10">
              {step.icon ? (
                <div className={`w-12 h-12 flex items-center justify-center rounded-full border-2 ${stateClass}`}>
                  {step.icon}
                </div>
              ) : (
                <div className={`w-12 h-12 flex items-center justify-center rounded-full border-2 ${stateClass}`}>
                  {withNumbers && (
                    <span className={`text-lg font-bold ${isCompleted ? 'text-white' : ''}`}>{index + 1}</span>
                  )}
                </div>
              )}
            </div>
            ...
            <div className={`mt-3 ${compact ? 'w-24' : 'w-full'} text-center`}>
              ...
              {step.date && (
                <Typography variant="small" className={`${colorClass.split(' ')[0]} font-medium mt-1`}>
                  {step.date}
                </Typography>
              )}
            </div>
    ...
    return (
      <div key={step.id} className="relative pl-10">
        ...
        <div className="absolute left-0 top-0">
          {step.icon ? (
            <div className={`w-10 h-10 flex items-center justify-center rounded-full border-2 ${stateClass}`}>
              {step.icon}
            </div>
          ) : (
            <div className={`w-10 h-10 flex items-center justify-center rounded-full border-2 ${stateClass}`}>
              {withNumbers && (
                <span className={`text-md font-bold ${isCompleted ? 'text-white' : ''}`}>{index + 1}</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="pb-8">
          <div className="flex items-center mb-1">
            ...
            {step.date && (
              <Typography variant="small" className={`${colorClass.split(' ')[0]} font-medium ml-2`}>
                {step.date}
              </Typography>
            )}
          </div>
