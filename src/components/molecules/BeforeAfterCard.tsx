    ...
    >
      <div className="flex items-center gap-4 mb-6">
        ...
        >
          <Icon 
            name={item.icon} 
            className={`w-6 h-6 text-[var(--color-${color})]`} 
          />
        </div>
        {showIndex && (
          <div>
            <Typography 
              variant="small" 
              className={`text-[var(--color-${color})] mb-1`}
            >
              Transformation {index! + 1}
            </Typography>
            <Typography variant="h4" className="font-bold">
              {item.icon === 'Clock' ? 'Gain de temps' : 
               item.icon === 'Users' ? 'Expérience client' :
               item.icon === 'Shield' ? 'Sécurité' : 
               'Amélioration'}
            </Typography>
          </div>
        )}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <Icon 
              name="Check" 
              className={`w-4 h-4 text-[var(--color-${color})]`} 
            />
            <Typography 
              variant="small" 
              className={`font-medium text-[var(--color-${color})]`}
            >
              Après
            </Typography>
          </div>
