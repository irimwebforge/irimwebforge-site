...
import { ContactForm, ContactFormData } from '@/components/molecules/ContactForm';
...
export const EnhancedContactForm: React.FC<EnhancedContactFormProps> = ({
  ...
  const renderContent = () => {
    ...
    const servicesSection = servicesToDisplay.length > 0 ? (
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          {servicesToDisplay.map((service, index) => (
            <ServiceHighlight
              key={index}
              icon={<img src={service.icon} alt={service.title} width={24} height={24} />}
              title={service.title}
    ...
    return (
      <>
        <div className={`flex flex-col ${imagePosition === 'left' ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-8`}>
          {formSection}
          {imageSection}
        </div>
